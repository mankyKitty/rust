...
// See above. Having two vectors exposed will only cause confusion, but it can be used internally.
#[doc(hidden)];
...
impl<T> Vec<T> {
    ...
    pub fn reserve(&mut self, capacity: uint) {
        if capacity >= self.len {
            ...
            self.cap = capacity;
            unsafe {
                self.ptr = realloc_raw(self.ptr as *mut c_void, size) as *mut T;
            }
        }
    }
    ...
    pub fn shrink_to_fit(&mut self) {
        unsafe {
            if self.len == 0 {
                ...
                self.ptr = realloc_raw(self.ptr as *mut c_void, self.len * size_of::<T>()) as *mut T;
                self.cap = self.len;
            }
        }
    }
